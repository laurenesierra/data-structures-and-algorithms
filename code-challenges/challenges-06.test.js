...
const templatingWithMustache = () => {
  ...
  characters.forEach((character) => {
    ...
    const renderHtml = Mustache.render(characterTemplate, character);
    stringTemplate.push(renderHtml);
...
const getHouses = (arr) => {
  let houses = [];
  const valueArr = Object.values(arr);
...
const hasChildrenValues = (arr, character) => {
  const takeInArray = Object.values(arr);
  let trueVar;
  takeInArray.forEach(getCharacter => {
    if (getCharacter.name === character) {
      const newArray = Array.from(getCharacter.children);
