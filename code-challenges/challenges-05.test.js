...
const templateWithJQuery = () => {
  starWarsPeople.forEach(person => {
    const getTemplate = $('#template').clone();
    const $h2 = getTemplate.find('h2');
    $h2.text(person.name);
    const $h3 = getTemplate.find('h3');
    $h3.text(person.height);
    const $p = getTemplate.find('p');
    $p.text(person.eye_color);
    getTemplate.removeAttr('id');
    $('main').append(getTemplate);
  });
};
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= str.length; i++) {
    const pencilSharpener = str.slice(i);
    result.push(pencilSharpener);
  }
  return result;
};
...
const wordsToCharList = (arr) => {
  const newArray = [];
  let stringIt = arr.split('');
  newArray.push(stringIt);
  return newArray[0];
};
